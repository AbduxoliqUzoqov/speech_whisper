...
class DataCollatorSpeechSeq2SeqWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        # BOS tokenni qirqib tashlash (takrorlanmasligi uchun)
        if (labels[:, 0] == self.processor.tokenizer.bos_token_id).all().cpu().item():
            labels = labels[:, 1:]
            
        batch["labels"] = labels
        
        # Xatolikni "temirbeton" qilib tuzatish: decoder_input_ids ni qo'lda yaratib uzatamiz
        decoder_input_ids = labels.new_zeros(labels.shape)
        decoder_input_ids[:, 1:] = labels[:, :-1].clone()
        decoder_input_ids[:, 0] = self.processor.tokenizer.bos_token_id
        decoder_input_ids.masked_fill_(decoder_input_ids == -100, self.processor.tokenizer.pad_token_id)
